...
3. Choose `InAppSettingsKit` only if you don't use SwiftUI, only `InAppSettingsKitSwiftUI` if you use it. The later include the original library (written in ObjC). You can totally write your own SwiftUI wrapper (and thus not need the `SwiftUI` Library) for more advanced stuff, but we built it for your ease of integration.
